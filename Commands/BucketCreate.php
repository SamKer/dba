...
class BucketCreate extends Command
{
    ...
    {
        $this->setName("bucket:create")
            ->setDescription('create buckets')
            ->addArgument("target", InputArgument::REQUIRED, "target name", null)
            ->addArgument("bucketName", InputArgument::REQUIRED, "Name of the bucket", null);
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $io->title("Buckets creator $target");
        ...
        //create bucket
        $list = $archiver->createBucket($bucketName);

                $io->writeln('Bucket created!');
                $io->writeln('---');
